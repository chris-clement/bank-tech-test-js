...
describe('bankAccountDetails class', () => {
  ...
  describe('#formatHistoryNumbers', () => {
    it('converts credits into 2dp string, debit to blank', () => {
      expect(bankDetails.formatHistoryNumbers([{credit: 100, debit: 0}])).toEqual([{credit: '100.00', debit: ''}]);
    });
    it('converts credits into blank string if 0, debit to 2dp', () => {
      expect(bankDetails.formatHistoryNumbers([{credit: 0, debit: 100}])).toEqual([{credit: '', debit: '100.00'}]);
    });
  });
});
